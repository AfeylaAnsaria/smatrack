...
@section('page-subtitle', 'Kelola data seluruh guru')

@section('content')
<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;">
    <div>
        <h1 style="font-size:24px;font-weight:800;">Data Guru</h1>
        <p class="text-muted">Total {{ $guru->total() }} guru terdaftar</p>
    </div>
...
<div class="card">
    <div class="table-wrapper">
        <table>
            <thead>
                <tr><th>#</th><th>Nama Guru</th><th>NIP</th><th>Email</th><th>No. HP</th><th>Login</th></tr>
            </thead>
            <tbody>
                @forelse($guru as $i => $g)
                <tr>
                    <td style="color:var(--text-muted);font-size:12px;">{{ $guru->firstItem() + $i }}</td>
                    <td>
                        <div style="display:flex;align-items:center;gap:10px;">
                            <div style="width:36px;height:36px;border-radius:50%;background:linear-gradient(135deg,#d6bcfa,#805ad5);color:white;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:14px;">
                                {{ strtoupper(substr($g->name,0,1)) }}
                            </div>
                            <div>
                                <div style="font-weight:600;">{{ $g->name }}</div>
                                <div style="font-size:11px;color:var(--text-muted);">Guru</div>
                            </div>
                        </div>
                    </td>
                    <td><span class="badge badge-purple">{{ $g->nip ?? '-' }}</span></td>
                    <td style="font-size:13px;">{{ $g->email }}</td>
                    <td style="font-size:13px;">{{ $g->no_hp ?? '-' }}</td>
                    <td><span class="badge badge-green"><i class="fas fa-check-circle"></i> &nbsp;Aktif</span></td>
                </tr>
                @empty
                <tr><td colspan="6"><div class="empty-state"><i class="fas fa-chalkboard-teacher"></i><p>Belum ada guru</p></div></td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    <div style="margin-top:16px;">{{ $guru->links() }}</div>
</div>
